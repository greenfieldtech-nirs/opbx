...
/**
 * Form request validator for updating an AI Assistant Load Balancer.
 */
class UpdateAlbsRequest extends FormRequest
{
    /**
    ...
    public function authorize(): bool
    {
        ...
        // Only Owner and PBX Admin can update load balancers
    ...
    {
        $user = $this->user();
        $loadBalancerId = $this->route('ai_assistant_load_balancer');
        ...
        return [
            'name' => [
                ...
                'min:2',
                'max:255',
                // Name must be unique within the organization, excluding current record
                Rule::unique('ai_assistant_load_balancers', 'name')
                    ->where(function ($query) use ($user) {
                        return $query->where('organization_id', $user->organization_id);
                    })
                    ->ignore($loadBalancerId),
            ],
            'description' => [
                'nullable',
                'string',
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            // Load the existing load balancer to check for self-reference
            $existingLoadBalancer = AiAssistantLoadBalancer::find($loadBalancerId);
            ...
            // Check for self-reference: cannot set own load balancer as fallback
            if ($fallbackAction === RingGroupFallbackAction::RING_GROUP->value && $fallbackRingGroupId) {
                // Note: In the future if we allow ALBS to reference other ALBS, check here
                // For now, ring groups can't reference ALBS
            }
