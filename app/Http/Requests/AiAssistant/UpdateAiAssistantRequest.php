...
/**
 * Form request validator for updating an AI assistant.
 */
class UpdateAiAssistantRequest extends FormRequest
{
    ...
    {
        $user = $this->user();
        $aiAssistantId = $this->route('ai_assistant');
        ...
        return [
            'name' => [
                ...
                'string',
                'max:255',
                // Name must be unique within the organization, excluding current assistant
                Rule::unique('ai_assistants', 'name')
                    ->where(function ($query) use ($user) {
                        return $query->where('organization_id', $user->organization_id)
                            ->whereNull('deleted_at');
                    })
                    ->ignore($aiAssistantId),
            ],
            'description' => [
                'nullable',
                'string',
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            // Validate provider exists in registry (if provided)
            ...
            // Validate configuration against provider definition (if provided)
