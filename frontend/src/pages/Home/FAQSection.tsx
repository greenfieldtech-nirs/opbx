      ...
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-border bg-card/50 mb-6">
            ...
            <span className="text-xl font-medium text-muted-foreground">FAQ</span>
          </div>
          <h2 className="text-3xl md:text-5xl font-bold mb-4 text-foreground">
            Frequently Asked Questions
          </h2>
          <p className="text-2xl text-muted-foreground">Everything you need to know about OPBX</p>
        </div>
        <div className="space-y-4">
          {faqs.map((faq, idx) => (
            <Card
              key={idx}
              className="cursor-pointer bg-card border-border hover:border-primary/50 transition-all"
              onClick={() => setOpenFaq(openFaq === idx ? null : idx)}
            >
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
                <CardTitle className="text-2xl font-semibold pr-8 text-foreground">
                  {faq.question}
                </CardTitle>
                <ChevronDown
