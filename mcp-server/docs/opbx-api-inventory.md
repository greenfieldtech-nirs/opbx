...
15. ~~**Campaign, session-update and call-notification route groups are Sanctum-only**~~ **RESOLVED** (2026-09-08): all business route groups now carry `resolve.api.key` + `enforce.api.key.scope`; 23 resources are grantable (incl. campaigns, distribution-lists, session-updates, call-notifications, ivr-menus, call-tracking-*, supervisors, ai-assistant-providers). Credential subroutes (`extensions.password`, `users.embed-token.*`, …) are explicitly excluded.
