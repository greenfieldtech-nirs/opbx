...
- **Role-based access control** supporting Owner, PBX Admin, Supervisor, PBX User, and Reporter roles
...
- **Supervisor role** for read-only, scoped monitoring of specific users and ring groups
- **Live call coaching** with spy (listen), whisper, and barge modes for Owners and Supervisors
- **In-browser Web Phone** (JsSIP/WebRTC softphone) for placing, receiving, and coaching calls
...
| Supervisor | Scoped monitor | Read-only view of assigned users/ring groups; coach live calls (spy/whisper/barge); use the Web Phone; no configuration or exports |
