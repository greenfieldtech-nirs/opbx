...
1. Open the extension's create or edit dialog
2. Select a DID under **Default Caller ID** (or leave it unset)
3. Save

An extension's Default Caller ID takes precedence over the caller ID configured on an [Outbound Whitelist](/modules/outbound-whitelist) rule. See the [Outbound Calling Overview](/installation/outbound-calling-overview) for the full precedence order and rationale.
...
- [Phone Numbers](/modules/phone-numbers) — DIDs route to extensions and serve as caller IDs
...
- [Outbound Calling Overview](/installation/outbound-calling-overview) — How the per-extension caller ID is used
