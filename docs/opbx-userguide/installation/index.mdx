...
| 8181 | Dialer worker webhook listener (container port 8081) |
| 8080 | MCP server (AI-agent API; configurable via `MCP_PORT`) |
...
| `dialer-worker` | `opbx_dialer_worker` | Go auto-dialer worker | 8181 (host) → 8081 (container) |
| `amd-worker` | `opbx_amd_worker` | Java/Vert.x AMD worker | *(internal)* |
| `ngrok` | `opbx_ngrok` | Webhook tunnel (dev only) | 4040 |
| `mcp-server` | `opbx_mcp` | MCP server for AI agents (see [MCP Server](/modules/mcp-server)) | 8080 |
...
```bash
APP_PORT=8080        # Use port 8080 instead of 80 (but see MCP_PORT below)
FRONTEND_PORT=3001   # Use port 3001 instead of 3000
```

Available port overrides: `APP_PORT` (nginx), `FRONTEND_PORT` (Vite), `MCP_PORT` (MCP server, default 8080), `DIALER_WEBHOOK_HOST_PORT` (dialer worker webhooks, default 8181).

### MySQL Slow Start

### "No such image" Errors (podman-compose)

Podman-compose does **not** build images automatically on `docker compose up` the way Docker Compose does. If a service fails with `no such image: ...-mcp-server` (or another locally-built service), build it explicitly first:

```bash
docker compose build mcp-server   # or: docker compose build
docker compose up -d mcp-server
```

Using `docker compose up -d --build` also works.
...
Common service names: `app`, `nginx`, `mysql`, `redis`, `queue-worker`, `dialer-worker`, `amd-worker`, `mcp-server`.
