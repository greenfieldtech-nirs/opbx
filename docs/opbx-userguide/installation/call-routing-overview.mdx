---
sidebar_position: 5
slug: /installation/call-routing-overview
...
```mermaid
graph LR
    A[Phone Number] --> B[IVR Menu]
    B --> C[Ring Group]
    C --> D[Fallback Extension]
```
...
```mermaid
graph LR
    A[Phone Number] --> B{Business Hours}
    B -->|Open| C[Ring Group]
    B -->|Closed| D[IVR]
```
...
```mermaid
graph TD
    A[Incoming Call] --> B["&lt;Gather&gt; - Play IVR menu, wait for digits"]
    B --> C["&lt;Dial&gt; - Route to extension based on input"]
    C --> D["&lt;Hangup&gt; - End when call completes"]
```
...
See [Phone Numbers (DIDs)](../modules/phone-numbers) for details.

### Ring Groups

Distribute calls to multiple extensions with various strategies.

See [Ring Groups](../modules/ring-groups) for details.

### IVR Menus

Create interactive menus for caller self-service.

See [IVR Menus](../modules/ivr-menus) for details.

### Business Hours

Route based on time of day and day of week.

See [Business Hours](../modules/business-hours) for details.
