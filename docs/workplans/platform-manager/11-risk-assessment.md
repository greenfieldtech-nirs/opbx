...
| **RISK-07** | **Token leakage with platform abilities.** PM token exposed in logs or client code. | Low | Critical | 10 | (a) Short token TTLs. (b) Token values never in audit logs. (c) Token revocation on PM flag removal. (d) Rate limiting. |
...
| **RISK-11** | **Audit log table growth.** Unbounded table growth over months. | Medium | Low | 6 | (a) Indexed on `created_at`. (b) Add retention policy command for future. (c) Pagination enforced. |
...
**Top Priority Risks (Score ≥ 10):** RISK-01 (20), RISK-03 (12), RISK-02 (10), RISK-06 (10), RISK-07 (10), RISK-09 (10)
